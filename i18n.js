export const translations = {
  uk: {
    appTitle: 'Калькулятор перекупа',
    loginTitle: 'Вхід у кабінет',
    password: 'Пароль',
    passwordPlaceholder: 'мінімум 6 символів',
    loginBtn: 'Увійти',
    noAccount: 'Немає акаунта?',
    registerLink: 'Зареєструватися',
    haveAccount: 'Вже маєте акаунт?',
    loginLink: 'Увійти',
    registerTitle: 'Реєстрація',
    registerBtn: 'Зареєструватися',
    themeDark: 'Темна',
    themeLight: 'Світла',
    themePurple: 'Фіолетова',
    bgBtn: 'Фон',
    bgReset: 'Скинути фон',
    logoutBtn: 'Вийти',
    newProjectBtn: '+ Новий проект',
    projectName: 'Назва проекту',
    save: 'Зберегти',
    currency: 'Валюта',
    addCarTitle: 'Додати авто',
    carName: 'Назва авто',
    buyPrice: 'Ціна покупки',
    sellPrice: 'Ціна продажу',
    notSoldYet: 'Ще не продано (без ціни продажу)',
    carComment: 'Коментар до покупки (опціонально)',
    carCommentPlaceholder: 'Наприклад: потрібна фарба',
    categoryLegendTitle: 'Позначення категорій:',
    carIconEditorTitle: 'Іконка авто',
    carIconUploadBtn: 'Завантажити своє фото',
    carIconResetBtn: 'Скинути',
    addCarBtn: 'Додати в історію',
    historyTitle: 'Історія авто',
    thName: 'Назва',
    thDate: 'Дата',
    dateBuyLabel: 'Купівля',
    dateSellLabel: 'Продаж',
    thBuy: 'Покупка',
    thSell: 'Продаж',
    thAdjust: 'Правки',
    thTotalCost: 'Загальні витрати',
    thProfit: 'Чистий плюс',
    thStatus: 'Статус',
    emptyHistory: 'Ще немає жодного авто в цьому проекті',
    totalLabel: 'Загальний заробіток по проекту:',
    statusSold: 'Продано',
    statusNotSold: 'В наявності',
    markSold: 'Позначити продано',
    sellPricePlaceholder: 'Ціна продажу',
    confirmSaleBtn: 'ОК',
    adjustModalTitle: 'Правки до авто',
    adjDesc: 'Опис',
    adjAmount: 'Сума',
    adjDescPlaceholder: 'Наприклад: покраска',
    adjAddBtn: 'Додати',
    commentsTitle: 'Коментарі до продажу',
    commentPlaceholder: 'Ваш коментар...',
    commentAddBtn: 'Додати',
    close: 'Закрити',
    deleteProjectConfirm: 'Видалити цей проект разом з історією?',
    deleteCarConfirm: 'Видалити це авто з історії?',
    newProjectPrompt: 'Назва нового проекту:',
    editComment: 'Правки/коментарі',
    sellPricePrompt: 'Введіть ціну продажу:',
    settingsTitle: 'Налаштування',
    settingsLang: 'Мова',
    settingsTheme: 'Тема',
    settingsBg: 'Фон',
    bgEdit: 'Редагувати',
    bgEditorTitle: 'Налаштування фону',
    bgEditorHint: 'Перетягуйте фото мишкою, масштаб — повзунком',
    cancel: 'Скасувати',
    settingsBgType: 'Тип фону',
    bgTypeVideo: 'Стандартне відео',
    bgTypeImage: 'Своє зображення',
    bgTypeCustomVideo: 'Своє відео/GIF',
    saveError: 'Не вдалося зберегти дані. Перевірте інтернет-з\'єднання і спробуйте ще раз.',
    imageProcessError: 'Не вдалося обробити зображення. Спробуйте інший файл.',
    settingsBgVideo: 'Своє відео/GIF',
    bgVideoBtn: 'Завантажити',
    bgVideoLimitsHint: 'Ліміти: файл до 20 МБ, відео до 15 секунд. Зберігається лише в цьому браузері (не синхронізується між пристроями).',
    bgVideoTooLarge: 'Файл занадто великий. Максимум 20 МБ.',
    bgVideoTooLong: 'Відео занадто довге. Максимум 15 секунд.',
    bgVideoWrongType: 'Непідтримуваний формат файлу. Дозволено: MP4, WebM, GIF.',
    settingsFit: 'Показ фону',
    fitCover: 'Заповнити екран',
    fitContain: 'Показати повністю',
    exportProjectTitle: 'Зберегти пресет проекту (файл)',
    importProjectTitle: 'Завантажити пресет проекту (файл)',
    importProjectSuccess: 'Проект успішно завантажено з файлу.',
    importProjectError: 'Не вдалося прочитати файл пресету. Перевірте, що це правильний файл.',
  },
  ru: {
    appTitle: 'Калькулятор перекупа',
    loginTitle: 'Вход в кабинет',
    password: 'Пароль',
    passwordPlaceholder: 'минимум 6 символов',
    loginBtn: 'Войти',
    noAccount: 'Нет аккаунта?',
    registerLink: 'Зарегистрироваться',
    haveAccount: 'Уже есть аккаунт?',
    loginLink: 'Войти',
    registerTitle: 'Регистрация',
    registerBtn: 'Зарегистрироваться',
    themeDark: 'Тёмная',
    themeLight: 'Светлая',
    themePurple: 'Фиолетовая',
    bgBtn: 'Фон',
    bgReset: 'Сбросить фон',
    logoutBtn: 'Выйти',
    newProjectBtn: '+ Новый проект',
    projectName: 'Название проекта',
    save: 'Сохранить',
    currency: 'Валюта',
    addCarTitle: 'Добавить авто',
    carName: 'Название авто',
    buyPrice: 'Цена покупки',
    sellPrice: 'Цена продажи',
    notSoldYet: 'Ещё не продано (без цены продажи)',
    carComment: 'Комментарий к покупке (опционально)',
    carCommentPlaceholder: 'Например: нужна покраска',
    categoryLegendTitle: 'Обозначения категорий:',
    carIconEditorTitle: 'Иконка авто',
    carIconUploadBtn: 'Загрузить своё фото',
    carIconResetBtn: 'Сбросить',
    addCarBtn: 'Добавить в историю',
    historyTitle: 'История авто',
    thName: 'Название',
    thDate: 'Дата',
    dateBuyLabel: 'Покупка',
    dateSellLabel: 'Продажа',
    thBuy: 'Покупка',
    thSell: 'Продажа',
    thAdjust: 'Правки',
    thTotalCost: 'Общие затраты',
    thProfit: 'Чистый плюс',
    thStatus: 'Статус',
    emptyHistory: 'Ещё нет ни одного авто в этом проекте',
    totalLabel: 'Общий заработок по проекту:',
    statusSold: 'Продано',
    statusNotSold: 'В наличии',
    markSold: 'Отметить продано',
    sellPricePlaceholder: 'Цена продажи',
    confirmSaleBtn: 'ОК',
    adjustModalTitle: 'Правки к авто',
    adjDesc: 'Описание',
    adjAmount: 'Сумма',
    adjDescPlaceholder: 'Например: покраска',
    adjAddBtn: 'Добавить',
    commentsTitle: 'Комментарии к продаже',
    commentPlaceholder: 'Ваш комментарий...',
    commentAddBtn: 'Добавить',
    close: 'Закрыть',
    deleteProjectConfirm: 'Удалить этот проект вместе с историей?',
    deleteCarConfirm: 'Удалить это авто из истории?',
    newProjectPrompt: 'Название нового проекта:',
    editComment: 'Правки/комментарии',
    sellPricePrompt: 'Введите цену продажи:',
    settingsTitle: 'Настройки',
    settingsLang: 'Язык',
    settingsTheme: 'Тема',
    settingsBg: 'Фон',
    bgEdit: 'Редактировать',
    bgEditorTitle: 'Настройка фона',
    bgEditorHint: 'Перетаскивайте фото мышкой, масштаб — ползунком',
    cancel: 'Отмена',
    settingsBgType: 'Тип фона',
    bgTypeVideo: 'Стандартное видео',
    bgTypeImage: 'Своё изображение',
    bgTypeCustomVideo: 'Своё видео/GIF',
    saveError: 'Не удалось сохранить данные. Проверьте интернет-соединение и попробуйте снова.',
    imageProcessError: 'Не удалось обработать изображение. Попробуйте другой файл.',
    settingsBgVideo: 'Своё видео/GIF',
    bgVideoBtn: 'Загрузить',
    bgVideoLimitsHint: 'Лимиты: файл до 20 МБ, видео до 15 секунд. Сохраняется только в этом браузере (не синхронизируется между устройствами).',
    bgVideoTooLarge: 'Файл слишком большой. Максимум 20 МБ.',
    bgVideoTooLong: 'Видео слишком длинное. Максимум 15 секунд.',
    bgVideoWrongType: 'Неподдерживаемый формат файла. Разрешено: MP4, WebM, GIF.',
    settingsFit: 'Показ фона',
    fitCover: 'Заполнить экран',
    fitContain: 'Показать полностью',
    exportProjectTitle: 'Сохранить пресет проекта (файл)',
    importProjectTitle: 'Загрузить пресет проекта (файл)',
    importProjectSuccess: 'Проект успешно загружен из файла.',
    importProjectError: 'Не удалось прочитать файл пресета. Проверьте, что это правильный файл.',
  }
};

export const currencySymbols = {
  UAH: '₴',
  USD: '$',
  EUR: '€',
  RUB: '₽'
};

export const categoryLabels = {
  uk: {
    econom: 'Економ клас',
    middle: 'Середній клас',
    premium: 'Преміум клас',
    trucks: 'Вантажний транспорт',
    moto: 'Мото-вело салон',
    water: 'Водний транспорт',
    air: 'Повітряний транспорт',
    rare: 'Рідкісні автомобілі',
    exclusive: 'Ексклюзивні автомобілі',
    rv: 'Дім на колесах'
  },
  ru: {
    econom: 'Эконом класс',
    middle: 'Средний класс',
    premium: 'Премиум класс',
    trucks: 'Грузовой транспорт',
    moto: 'Мото-вело салон',
    water: 'Водный транспорт',
    air: 'Воздушный транспорт',
    rare: 'Редкие автомобили',
    exclusive: 'Эксклюзивные автомобили',
    rv: 'Дом на колёсах'
  }
};

export function getCategoryLabel(categoryKey) {
  return (categoryLabels[currentLang] && categoryLabels[currentLang][categoryKey]) || categoryKey;
}

let currentLang = localStorage.getItem('appLang') || 'uk';

export function getLang() {
  return currentLang;
}

export function setLang(lang) {
  currentLang = lang;
  localStorage.setItem('appLang', lang);
  applyTranslations();
}

export function t(key) {
  return (translations[currentLang] && translations[currentLang][key]) || key;
}

export function applyTranslations() {
  document.querySelectorAll('[data-i18n]').forEach(el => {
    const key = el.getAttribute('data-i18n');
    el.textContent = t(key);
  });
  document.querySelectorAll('[data-i18n-placeholder]').forEach(el => {
    const key = el.getAttribute('data-i18n-placeholder');
    el.setAttribute('placeholder', t(key));
  });
  document.querySelectorAll('[data-i18n-title]').forEach(el => {
    const key = el.getAttribute('data-i18n-title');
    el.setAttribute('title', t(key));
  });
  document.documentElement.lang = currentLang;
}
